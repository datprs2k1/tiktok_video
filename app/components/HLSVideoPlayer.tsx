...
export default function HLSVideoPlayer({
  ...
  // Preload mechanism: track last preload position to prevent duplicate preloads
  const lastPreloadPositionRef = useRef<number>(0);
  const PRELOAD_THRESHOLD = 1.5; // seconds - preload when gap between currentTime and bufferedEnd is less than this
    ...
    if (Hls.isSupported()) {
      ...
      const handleSeeking = () => {
        ...
        if (video) {
          ...
          // Reset preload tracking on seek to allow preload for new position
          lastPreloadPositionRef.current = 0;
          debugLog(
  ...
  useEffect(() => {
    ...
    const handleTimeUpdate = () => {
      ...
      // Preload mechanism: trigger preload when near end of current segment
      const gap = bufferedEnd - currentTime;
      if (
        gap < PRELOAD_THRESHOLD &&
        currentTime > lastPreloadPositionRef.current &&
        !video.paused &&
        !isSeekingRef.current
      ) {
        // Trigger preload for next segment
        throttledStartLoad();
        lastPreloadPositionRef.current = currentTime;
        debugLog('[HLS] Preload triggered, gap:', gap.toFixed(2), 'currentTime:', currentTime.toFixed(2));
      }
